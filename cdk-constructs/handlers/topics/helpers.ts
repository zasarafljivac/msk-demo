...
export async function waitForMskActive(timeoutMs: number, region: string, clusterArn: string) {
  ...
  while (true) {
    ...
    if (Date.now() - start > timeoutMs) throw new Error('MSK cluster not ACTIVE in time');
...
export async function compactTopic(admin: Admin, topic: string) {
  ...
  const toMap = (arr: any[]) => Object.fromEntries(arr.map((e: any) => [e.name, e.value]));
  const cfg = toMap(entries);

  const needsCleanupPolicy = !cfg['cleanup.policy'] || !String(cfg['cleanup.policy']).includes('compact');
  ...
  const hasIncremental = typeof (admin as any).incrementalAlterConfigs === 'function';
