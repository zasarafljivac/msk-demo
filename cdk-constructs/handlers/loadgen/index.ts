...
import type { Envelope as XEnvelope } from '../transform/index';
import { ids } from './ids';

type Envelope = Omit<XEnvelope, 'key'>;
...
function parseMix(s?: string): MixWeights {
  ...
  const total =
    (parts.orders ?? 0) + (parts.shipments ?? 0) + (parts.events ?? 0) + (parts.invoices ?? 0) || 1;
...
function planBatch(n: number, weights: MixWeights): Record<MixKey, number> {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (r < weights.orders) {
      counts.orders++;
    } else if (r < weights.orders + weights.shipments) {
      counts.shipments++;
...
function makeSecondBatch(rate: number, mix: MixWeights, deletePct: number): Envelope[] {
  ...
  for (let i = 0; i < counts.invoices; i++) {
    ...
    if (del) {
      ...
      const invoiceIdStr = header.data!.invoice_id as string;
      const { msgs: lineMsgs, total } = buildInvoiceItems(invoiceIdStr);
...
export async function handler(event: LoadgenEvent = {} as LoadgenEvent): Promise<{
  topic: string;
  rate: number;
  seconds: number;
  sent: number;
  ...
  if (!BOOTSTRAP.length) {
    throw new Error('Missing BOOTSTRAP_BROKERS_SASL_IAM');
  }
