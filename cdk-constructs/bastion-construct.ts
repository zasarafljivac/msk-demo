...
import {
  Instance, InstanceClass, InstanceSize, InstanceType, IVpc, ISecurityGroup,
  MachineImage, SubnetType,
  CloudFormationInit,
  InitCommand,
  InitPackage,
  InitFile
...
export class BastionConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string, props: BastionConstructProps) {
    ...
    this.instance = new Instance(this, 'Bastion', {
      vpc: props.vpc,
      vpcSubnets: { subnetType: props.subnetsType ?? SubnetType.PRIVATE_WITH_EGRESS },
    ...
    const init = CloudFormationInit.fromElements(
      ...
      InitCommand.shellCommand('KVER=3.8.1 SCALA=2.13; '
      ...
      InitFile.fromString('/opt/kafka/client.properties', [
      ...
      InitFile.fromString('/etc/profile.d/kafka.sh',
