...
export class LambdaConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string, props: LambdaConstructProps) {
        ...
        commandHooks: {
          ...
          afterBundling(inputDir: string, outputDir: string) {
            const hostRel = path
              .relative(
                cwd(),
                path.join(path.dirname(schemaEntry), schemaFileRelFromEntryDir),
              )
              .replace(/\\/g, "/");
            const srcInContainer = `${inputDir}/${hostRel}`;
            return [`cp "${srcInContainer}" "${outputDir}/schema.sql"`];
          },
        },
      },
      entry: schemaEntry,
    ...
    const layer = LayerVersion.fromLayerVersionArn(
      this,
      "SecretLayer",
      "arn:aws:lambda:us-east-1:177933569100:layer:AWS-Parameters-and-Secrets-Lambda-Extension:18",
    );
